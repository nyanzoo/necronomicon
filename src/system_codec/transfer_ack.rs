...
impl<R> PartialDecode<R> for TransferAck
where
    R: Read,
{
...
mod test {
    ...
    fn test_encode_decode() {
        let header = Header::new(Kind::TransferAck, 123, 456);
        let mut buf = Vec::new();
        let chain_ack = TransferAck {
            header,
            response_code: 0,
        };
        ...
        let decoded = TransferAck::decode(header, &mut buf).unwrap();
